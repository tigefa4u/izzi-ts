...
const validateAndJoinRaid = async ({ user_tag, raidId }: T) => {
	...
	const user = await getRPGUser({ user_tag }, { cached: true });
	if (!user) return;
...
export const raidRecruit = async ({ context, client, options }: BaseProps) => {
	try {
		...
		if (!guildEvent || !role || !channel) {
			embed.setDescription(`Summoner **${author.username}**, a Raid Ping or Channel ` +
		...
		if (!hasPermission) {
			embed.setDescription(`Permissions missing from channel **<#${channel}>**`);
			context.channel?.sendMessage(embed);
			return;
		}
		...
		if (!currentRaid) {
			embed.setDescription(`Summoner **${author.username}**, you are currently not in a raid! ` +
            "use ``iz rd spawn`` to spawn one or join an existing raid.");
			context.channel?.sendMessage(embed);
			return;
		}
		if (currentRaid.is_start) {
			embed.setDescription(`Summoner **${author.username}**, the raid challenge has already started!`);
			context.channel?.sendMessage(embed);
			return;
		}
		...
		if (!member) return;
		if (!member.is_leader) {
			embed.setDescription(`Summoner **${author.username}**, only the lobby leader can use this command.`);
			context.channel?.sendMessage(embed);
			return;
		}
		const slotsLeft = MAX_RAID_LOBBY_MEMBERS - Object.keys(currentRaid.lobby).length;
		if (slotsLeft <= 0) {
			embed.setDescription(`Summoner **${author.username}**, there are currently no slots left in your raid. ` +
            "Use ``iz rd start`` to start the Raid Challenge!");
			context.channel?.sendMessage(embed);
			return;
		}
		const sendInChannel = await context.guild.channels.fetch(channel) as BaseProps["context"]["channel"];
		if (!sendInChannel) {
			context.channel?.sendMessage("Could not find configured channel. Please reconfigure.");
			return;
		}
		...
		const raidPingMessage = `All <@&${guildEvent.metadata.role}> Raiders assemble! ` +
