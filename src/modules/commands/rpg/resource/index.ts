...
import { getMentionedUser, randomElementFromArray } from "helpers";
...
export const give = async ({ context, client, options, args }: BaseProps) => {
	try {
		...
		const mentionedUser = await getMentionedUser(context);
		if (!mentionedUser) {
			embed.setDescription("Could not find this user in the Xenverse");
			context.channel.sendMessage(embed);
			return;
		}
