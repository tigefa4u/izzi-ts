...
export const getWishlist = async (
	...
	try {
		...
		loggers.error(
			"api.controllers.WishlistsController.getWishlist: ERROR",
			err
		);
		return;
	}
...
export const createWishlist = (data: WishlistCreateProps) => {
	try {
		loggers.info("Creating wishlist with data: ", { data });
		return Wishlist.create(data);
	} catch (err) {
		loggers.error(
			"api.controllers.WishlistsController.createWishlist: ERROR",
			err
		);
		return;
	}
...
export const updateWishlist = (
	params: WishlistUpdateParamProps,
	data: WishlistUpdateProps
) => {
	try {
		return Wishlist.update(params, data);
	} catch (err) {
		loggers.error(
			"api.controllers.WishlistsController.updateWishlist: ERROR",
			err
		);
		return;
	}
...
export const removeFromWishlist = (params: WishlistUpdateParamProps) => {
	try {
		...
		loggers.error(
			"api.controllers.WishlistsController.removeFromWishlist: ERROR",
			err
		);
		return;
	}
...
export const getWishlistBySkinId = (params: {
  user_tag: string;
  skin_id: number;
}) => {
	try {
		...
		loggers.error(
			"api.controllers.WishlistsController.getWishlistBySkinId: ERROR",
			err
		);
		return;
	}
...
export const getWishlistById = (params: { id: number; user_tag: string }) => {
	try {
		...
		loggers.error(
			"api.controllers.WishlistsController.getWishlistById: ERROR",
			err
		);
		return;
	}
...
export const getRandomCardFromWlist = async (params: {
  user_tag: string;
  is_referral_card: boolean;
  is_xenex_card: boolean;
}): Promise<WishlistProps[] | undefined> => {
	try {
		return Wishlist.getRandom(params);
	} catch (err) {
		loggers.error(
			"controllers.WishlistsController.getRandomCardFromWlist: ERROR",
			err
		);
		return;
	}
